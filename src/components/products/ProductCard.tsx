...
const ProductCard = ({ product, index = 0, showCompare = true }: ProductCardProps) => {
  ...
  return (
    <div
      className="group card-oneair overflow-hidden opacity-0 animate-[slide-up_0.6s_ease-out_forwards] flex flex-col h-full"
      ...
      <div className="flex flex-col flex-grow space-y-1.5">
        ...
        <div className="flex items-center gap-1.5">
          <div className="flex items-center">
            {[...Array(5)].map((_, i) => (
              <Star key={i} className={`h-3 w-3 ${i < Math.floor(product.rating) ? "fill-oneair-gold text-oneair-gold" : "text-muted"}`} />
            ))}
          </div>
